@extends('root')
@section('title', 'Laporan Keterlambatan dan Pulang Cepat Pegawai')
@section('content')
...
<div class="">
    @include('components.breadcrumb', [
        'title' => 'Laporan Keterlambatan dan Pulang Cepat Pegawai',
        'links' => [['url' => route('dashboard'), 'label' => 'Laporan']],
        'current' => 'Laporan Keterlambatan dan Pulang Cepat Pegawai',
    ])
        ...
        <div class="card card-body">
            <div class="wrap-table">
                <div class="table-responsive">
                    <h4 class="text-center mb-4">Laporan Keterlambatan dan Pulang Cepat Pegawai</h4>
                    ...
                    <table class="table table-bordered table-striped" id="absensi-table">
                        <thead>
                            <tr>
                                ...
                                <th>Rata-rata Pulang Cepat</th>
                                <th>Total Jam Kerja Berkurang</th>
                            </tr>
                        </thead>
                        <tbody id="keterlambatan-table-body">
...
<script>
    ...
    $.ajax({
        url: '{{ route('laporan.getLaporanKeterlambatan') }}', // Endpoint API untuk laporan keterlambatan
        method: 'GET',
        data: {
            divisi_id: divisiId,
            month: bulan,
        ...
        success: function (response) {
            const tbody = $('#keterlambatan-table-body');
            ...
            if (response.success) {
                ...
                const emptyRow = `
                    <tr>
                        <td colspan="6" class="text-center">Harap pilih divisi terlebih dahulu</td>
                    </tr>
                `;
                tbody.append(emptyRow);
            }
        ...
        error: function () {
            alert('Terjadi kesalahan saat memuat data keterlambatan.');
        }
    });
}


function renderKeterlambatanTable(data) {
    const tbody = $('#keterlambatan-table-body');
    tbody.empty();
    ...
    data.forEach((pegawai, index) => {
        const row = `
            <tr>
                ...
                <td>${pegawai.total_hari_kerja || 0}</td>
                <td>${pegawai.total_terlambat || 0}</td>
                <td>${pegawai.rata_rata_keterlambatan || ''}</td>
                <td>${pegawai.total_pulang_cepat || ''}</td>
                <td>${pegawai.total_jam_kerja_berkurang || ''}</td>
                <td>${pegawai.rata_rata_pulang_cepat || ''}</td>
            </tr>
        `;
        tbody.append(row);
    });
