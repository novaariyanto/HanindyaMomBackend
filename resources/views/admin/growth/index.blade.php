...
<div class="container">
	@include('components.breadcrumb', [
		'title' => 'Data ' . ($title ?? 'Growth Logs'),
		'links' => [
			['url' => route('admin.growth.index'), 'label' => 'Growth Logs'],
		],
		'current' => ($title ?? 'Growth Logs')
	])
	<div class="card shadow-sm">
		<div class="card-body p-0">
			<div class="table-responsive">
				<table class="table table-bordered table-hover mb-0" id="growth-table" style="width:100%">
		...
		<div class="modal-dialog modal-lg modal-dialog-scrollable">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Form Growth</h5>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const table = $('#growth-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('admin.growth.index') }}",
            type: 'GET'
        },
        columns: [
            ...
            { data: 'head_circumference', name: 'head_circumference' },
            { data: 'action', name: 'action', orderable: false, searchable: false }
        ]
    });

    const modalEl = document.getElementById('growthModal');
    ...
    function openForm(url, title){
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
            ...
            .then(html => {
                modalEl.querySelector('.modal-title').textContent = title || 'Form Growth';
    ...
    createBtn.dataset.url = "{{ route('admin.growth.create') }}";
    ...
    $(document).on('click', '.btnn-create', function(e){
        ...
        openForm(url, 'Tambah/Update Growth');
    });
