@extends('root')
@section('title', 'Diaper Logs')

@section('content')
<div class="container">
	@include('components.breadcrumb', [
		'title' => 'Data ' . ($title ?? 'Diaper Logs'),
		'links' => [
			['url' => route('admin.diapers.index'), 'label' => 'Diaper Logs'],
		],
		'current' => ($title ?? 'Diaper Logs')
	])
	<div class="card shadow-sm">
		<div class="card-body p-0">
			<div class="table-responsive">
				<table class="table table-bordered table-hover mb-0" id="diapers-table" style="width:100%">
					<thead class="table-light"><tr><th>Baby</th><th>Type</th><th>Time</th><th>Color</th><th>Texture</th><th>Notes</th><th style="width:160px">Aksi</th></tr></thead>
					<tbody></tbody>
				</table>
			</div>
		</div>
	...
	<!-- Modal -->
	<div class="modal fade" id="diaperModal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-lg modal-dialog-scrollable">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Form Diaper</h5>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const table = $('#diapers-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('admin.diapers.index') }}",
            type: 'GET'
        },
        columns: [
    ...
    const modalEl = document.getElementById('diaperModal');
    ...
    function openForm(url, title){
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
            ...
            .then(html => {
                modalEl.querySelector('.modal-title').textContent = title || 'Form Diaper';
    ...
    createBtn.dataset.url = "{{ route('admin.diapers.create') }}";
    ...
    $(document).on('click', '.btnn-create', function(e){
        ...
        openForm(url, 'Tambah/Update Diaper');
    });
