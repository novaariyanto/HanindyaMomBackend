...
class AddKategoriNonMedisToSuperAdminSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        // Ambil menu Kategori Non Medis
        $kategoriNonMedisMenu = Menu::where('nama_menu', 'Kategori Non Medis')->first();
        
        if (!$kategoriNonMedisMenu) {
            $this->command->error('Menu Kategori Non Medis tidak ditemukan');
            return;
        }

        // Tambahkan menu ke role jika belum ada
        $existingRoleMenu = RoleMenu::where('role_id', $superAdminRole->id)
            ->where('menu_id', $kategoriNonMedisMenu->id)
            ->first();
            
        if (!$existingRoleMenu) {
            RoleMenu::create([
                'role_id' => $superAdminRole->id,
                'menu_id' => $kategoriNonMedisMenu->id
            ]);
            $this->command->info('Menu Kategori Non Medis berhasil ditambahkan ke role SUPERADMIN');
        } else {
            $this->command->info('Menu Kategori Non Medis sudah ada di role SUPERADMIN');
        }
